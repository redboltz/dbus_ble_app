	...
	gpointer user_data)
{
	...
	if (g_attrib_set_mtu(ud->attrib, mtu)) {
		...
		gatt_write_char(
			ud->attrib,
			ud->handle,
			reinterpret_cast<uint8_t const*>(sbuf.data()),
			sbuf.size(),
			gatt_write_char_cb,
			nullptr);
	}
...
int main(int argc, char *argv[])
{
	...
	user_data_t ud { handle , fname_content {fn, content} , ATT_DEFAULT_LE_MTU };
	if (argc == 6) {
