...
static void notify_cb(
	...
	size_t const payload_offset = 3;
	size_t const payload_size = len - payload_offset;
	ud->unp.reserve_buffer(payload_size);
	memcpy(ud->unp.buffer(), &pdu[payload_offset], payload_size);
	ud->unp.buffer_consumed(payload_size);
	msgpack::unpacked result;
	fname_content fc;
	if (ud->unp.next(&result)) {
		...
		std::cout << "file: " << fc.fname << " stored." << std::endl;
	}
}
